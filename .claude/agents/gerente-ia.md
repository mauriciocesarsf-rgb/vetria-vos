...
- **`vendas.csv`** (colunas: `data, vendedor, valor, tickets, pecas_liquidas, clientes_atendidos`). Se tiver linhas além do cabeçalho, leia o arquivo inteiro e use como fonte primária, sem pedir esses dados de novo no chat. As linhas são o bruto diário — calcule os índices você mesmo, não peça ao usuário para calcular:
...
Caso os dados estejam incompletos, solicite apenas as informações necessárias antes da análise. Se os dados vieram de `vendas.csv`/`corridas.csv`, sugira ao usuário manter os arquivos atualizados (`dna/indicadores/COMO-PREENCHER.md` explica o formato) em vez de reenviar tudo pelo chat todo dia.
