...
- **`meta-mensal-loja.csv`** (colunas: `mes, meta_loja, meta_super, bonificacao_cota_pct, bonificacao_super_pct`). Cota e, se houver, meta "super" (esticada) do mês, mais os percentuais de bonificação de cada uma. Referência para diagnósticos e planejamento mensal, e para calcular a meta individual automática das corridas de faturamento (ver acima). **Achado real, 2026-09-03: `/gerente-enviar-relatorio` (relatório diário automático) lê a meta de `corridas.csv` (linha `metrica=valor`), não deste arquivo — são duas representações da mesma meta.** Sempre que você registrar ou atualizar `meta_loja` aqui (pelo chat, nunca quando isso vem só da Área Adm — essa já sincroniza sozinha), crie ou atualize também a linha correspondente em `corridas.csv`: `nome` = "Meta de {mês por extenso}", `metrica=valor`, `periodo_inicio`/`periodo_fim` = mês inteiro, `meta_por_vendedor` = `meta_loja` ÷ vendedores ativos. Sem isso, a meta fica cadastrada mas o relatório diário nunca tem o que mostrar — já aconteceu de verdade numa loja.
...
- **`vendedores.json`** (campos: `id, nome, nomesAnteriores, funcao, dataInicio, dataInicioVendas, cor, emoji, telefone, horarioTrabalho, entrada, saida, sonhos, objetivos, valorVendasPessoal, ativo`). Formato JSON, não CSV — leia com `JSON.parse`. `cor` é tom de pele (`preto`/`branca`/`parda`), usado só na bolinha de avatar da Área Adm — não confunda com `emoji` (achado real, 2026-09-05): `emoji` é o ícone escolhido pelo próprio cadastro pra identificar a pessoa nos relatórios (ver `/gerente-enviar-relatorio` Passo 4), evita todo vendedor do mesmo gênero aparecer com o mesmo ícone genérico. `horarioTrabalho` é texto livre, informativo, sem cálculo em cima (ex: "9h às 18h, com 1h de almoço variável"); `entrada`/`saida` (achado real, 2026-09-14, campos `HH:mm`) são o turno estruturado, usado pra validação de cobertura na gestão de escala (ver seção "GESTÃO DE ESCALAS" abaixo) — quando ausentes, trate como turno não cadastrado, nunca infira a partir de `horarioTrabalho`. **Define quem está ativo na loja** (campo `ativo`) — isso substitui a regra antiga de "quem tem lançamento em vendas.csv". Ao casar um nome de `vendas.csv` com um perfil, confira também `nomesAnteriores` (alguém pode ter sido renomeado no cadastro). `vendas.csv` continua sendo o histórico de vendas, nunca duplique número de performance dentro do perfil. Se o arquivo não existir ou não tiver essa pessoa cadastrada ainda, trate como não fazendo parte do cálculo de meta individual até ser cadastrada — nunca invente um perfil.
...
Achado real, 2026-09-14: pedido do usuário pra deixar a gestão de escala mais robusta que só gerar a grade de folgas do mês. Aplica-se sempre que a conversa envolver `escala-{AAAA-MM}.csv`, `config-escala.md` ou o campo `entrada`/`saida` de `vendedores.json` — não é um comando novo, é uma forma de trabalhar que vale tanto dentro de `/gerente-sugerir-escala` quanto numa conversa livre pedindo pra consultar ou alterar a escala.

**Validar antes de apresentar.** Toda vez que uma escala (existente ou recém-sugerida) for mostrada, classifique:
...
**Simular antes de aplicar, quando a mudança for relevante** (trocar mais de uma folga, reorganizar uma semana inteira, mudar o padrão de rodízio). Formato:
```
SOLICITAÇÃO: {o que foi pedido}
IMPACTO: {quem e o que muda}
SOLUÇÃO PROPOSTA: {a alteração concreta}
RESULTADO: {como fica depois, validado}
REGRAS AFETADAS: {nenhuma, ou quais}
```
Pra uma troca simples e pontual (um dia, uma pessoa), pode aplicar direto e só confirmar o resultado, sem esse formato completo — ele é pra mudança que mexe em várias pessoas/dias de uma vez.

**Consultar escala.** Responda direto perguntas como "quem trabalha hoje", "quem folga domingo", "quantas pessoas estão de folga essa semana", "quem já folgou mais domingos esse mês" — os dados já estão em `escala-{AAAA-MM}.csv` e `vendedores.json`, não peça pra rodar nenhum comando separado pra isso.
